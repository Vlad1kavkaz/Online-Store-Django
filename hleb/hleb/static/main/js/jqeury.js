/*!
 * Библиотека JavaScript jQuery v3.7.0
 * https://jquery.com/
 *
...
( функция (глобальная, фабрика) {
	...
	if ( typeof module === "object" && typeof module.exports === "object" ) {
		...
		// См. тикет trac-14549 для получения дополнительной информации.
...
jQuery.extend( {
	...
	// Получить текстовое значение массива узлов DOM
	...
	isXMLDoc: функция (элемент) {
		...
		// Предположим, HTML, когда documentElement еще не существует, например, внутри
		// фрагменты документа.
...
переменная сортировка = обр.сортировка;


вар сращивания = arr.splice;
...
функция fcssescape(ch, asCodePoint) {
	если ( asCodePoint ) {
		...
		вернуть ch.slice( 0, -1 ) + "\\" + ch.charCodeAt( ch.length - 1 ).toString( 16 ) + " ";
	}
...
вар предпочитаемый документ = документ,
	pushNative = нажать;
...
вар я,
	...
	booleans = "проверено | выбрано | асинхронно | автофокус | автовоспроизведение | управление | отложить | отключено | скрыто | ismap |" +
		"цикл|несколько|открытый|только для чтения|обязательный|ограниченный",
	...
	rleadingCombinator = новое регулярное выражение ("^" + пробел + "*([>+~]|" + пробел + ")" +
		пробел + "*" ),
	...
	matchExpr = {
		...
		РЕБЕНОК: новое регулярное выражение (
			"^:(только|первый|последний|n-й|n-последний)-(дочерний|типа)(?:\\(" +
		...
		needContext: новое регулярное выражение («^» + пробел +
	...
	// Используется для фреймов; см. `setDocument`.
...
пытаться {
	...
	нажать = {
		...
		вызов: функция ( цель ) {
			pushNative.apply(цель, slice.call(аргументы, 1));
		}
	};
}

функция найти (селектор, контекст, результаты, семя) {
	...
	если (! семя) {
		...
		если (документIsHTML) {
			...
			if ( nodeType !== 11 && (match = rquickExpr.exec(селектор)) ) {
				...
				// Селектор класса
				} иначе если ( ( m = match[ 3 ] ) && context.getElementsByClassName ) {
			...
			если ( !nonnativeSelectorCache[ селектор + " " ] &&
				( !rbuggyQSA || !rbuggyQSA.test( селектор )) ) {

				новыйселектор = селектор;
				новыйконтекст = контекст;

				// qSA рассматривает элементы за пределами корня области видимости при оценке дочерних или
				...
				// Технику следует использовать и при использовании ведущего комбинатора
				...
				если (тип узла === 1 &&
					...
					// IE/Edge иногда выдает ошибку «Отказано в доступе», когда
...
функция создания кэша () {
	...
	кэш функций( ключ, значение ) {
		...
		// Используйте (клавиша + ""), чтобы избежать столкновения со свойствами нативного прототипа
...
/**
 * Пометить функцию для специального использования модулем селектора jQuery.
...
функция createButtonPseudo (тип) {
	функция возврата (элемент) {
		return (имя_узла(элемент, "ввод") || имя_узла(элемент, "кнопка")) &&
			элемент.тип === тип;
	};
}
...
/**
 * Проверяет узел на допустимость в качестве контекста селектора jQuery
...
функция setDocument(узел) {
	...
	документ = документ;
	элемент документа = элемент документа.элемент документа;
	documentIsHTML = !jQuery.isXMLDoc(документ);

	// Поддержка: только iOS 7, IE 9–11+
	// Старые браузеры не поддерживали совпадения без префикса.
	совпадения = documentElement.matches ||
		documentElement.webkitMatchesSelector ||
		documentElement.msMatchesSelector;

	// Поддержка: IE 9–11+, Edge 12–18+
	// При доступе к документам iframe после выгрузки возникают ошибки «Отказано в доступе» (см. trac-13936)
	// Поддержка: IE 11+, Edge 17–18+
	// IE/Edge иногда выдает ошибку «Отказано в доступе» при строгом сравнении
	// два документа; поверхностные сравнения работают.
	// eslint-disable-next-line eqeqeq
	...
	support.getById = утверждение (функция (эль) {
		...
		вернуть !document.getElementsByName ||
	...
	// на отключенном узле.
	support.disconnectedMatch = утверждение (функция (эль) {
		возврат match.call(el, "*");
	});

	// Поддержка: IE 9–11+, Edge 12–18+
	// IE/Edge не поддерживает псевдокласс :scope.
	support.scope = утверждение (функция () {
		вернуть document.querySelectorAll(":scope");
	});

	// Поддержка: только Chrome 105–111, только Safari 15.4–16.3
	// Убедиться, что аргумент `:has()` неумолимо проанализирован.
	// Мы включаем `*` в тест, чтобы обнаружить ошибочные реализации, которые
	// _выборочно_ снисходительно (особенно когда список включает как минимум
	// один допустимый селектор).
	// Обратите внимание, что мы относимся к полному отсутствию поддержки `:has()` так, как если бы это было
	// совместимая со спецификацией поддержка, что хорошо, потому что использование `:has()` в таких
	// среды не пройдут путь qSA и вернутся к обходу jQuery
	// в любом случае.
	support.cssHas = утверждение (функция () {
		пытаться {
	...
	если ( support.getById ) {
		Expr.filter.ID = функция (идентификатор) {
	...
	// Ярлык
	Expr.find.TAG = функция (тег, контекст) {
	...
	// Сорт
	Expr.find.CLASS = функция (имя класса, контекст) {
	...
	утверждать(функция(эль) {

		ввод переменной;

		documentElement.appendChild(эль).innerHTML =
			"<a id='" + expando + "' href='' disabled='disabled'></a>" +
			"<select id='" + expando + "-\r\\' disabled='disabled'>" +
			"<option selected=''></option></select>";

		// Поддержка: только iOS <=7–8
		// Логические атрибуты и "значение" обрабатываются неправильно в некоторых XML-документах
